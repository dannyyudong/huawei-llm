...
def main(argv):
  ...
  for func, output_file_name in [
    ...
    # Save the same report content to a text file for later review.
    ...
    report_file_name = os.path.join(
        _OUTPUT_DIR.value, f"{output_filename.replace('.jsonl', '')}-report.txt"
    )
    with open(report_file_name, "w", encoding="utf-8") as f:
      f.write(report_buffer.getvalue())
    logging.info("Saved report: %s", report_file_name)
